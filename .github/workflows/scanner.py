...
VOL_X = float(os.getenv("VOLUME_MULTIPLIER", "1.1"))  # Secret from GitHub Actions
    ...
    for i in range(max_retries):
        try:
            ...
            if r.status_code >= 500:
                ...
                print(f"[{r.status_code}] Server error. Retrying in {sleep:.2f}s")
                time.sleep(sleep)
                continue
            ...
            sleep = base_sleep * (2 ** i)
            print(f"[Exception] {url}: {e}, retrying in {sleep:.2f}s")
            time.sleep(sleep)
    return None
...
def main():
    ...
    for c in coins:
        ...
        volumes = [float(k[5]) for k in klines]        # volume column
        avg30 = sma(volumes[-31:-1])                  # last 30 closed candles
        curr = volumes[-2]                             # most recent closed candle
        scanned += 1
    ...
    # Force a test alert if nothing triggered (useful for debugging)
    if alerts_sent==0 and scanned>0:
        tg_send(f"✅ Test alert: Scanner ran at {now}, scanned {scanned} coins, no spike detected")
        print("✅ Test alert sent")

    print(f"== Scan completed: scanned={scanned}, alerts={alerts_sent} ==")
    return 0
